...
function ec(view, input) {
  ...
  var a = t * n * (B - input.width / 2) + D / 2;
  var e = t * n * (C - input.height / 2) + E / 2;
  a = Math.round(a);
  e = Math.round(e);
  var b = Math.round(input.width * t * n);
  var g = Math.round(input.height * t * n);
  if (ca) {
    if (((ya = !1), 0 <= input.I && 0 <= input.J)) {
      (b < D || g < E) &&
        Ha > n &&
